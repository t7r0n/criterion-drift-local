...
from criterion_drift_local.engine import (
    LocalHarness,
    benchmark,
    export_demo_pack,
    jsonl_loop,
...
from criterion_drift_local.fixtures import init_store, load_config, load_scenarios
...
def test_benchmark_and_cli_smoke():
    result = subprocess.run(
        [sys.executable, "-m", "criterion_drift_local.cli", "init-demo", "--force"],
        check=True,
        capture_output=True,
        text=True,
    )
